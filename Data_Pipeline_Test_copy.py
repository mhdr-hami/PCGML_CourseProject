...
def texter(address1, map_type):
    ...
    stacked_list=[]
    with open(s+address1) as f:
        # print("ssssss"+s+address1)
        for line in f :
            line_list=[]
            for element in line:
                if element != '\n':
...
def dataset_creator(address_1, map_type):
    dataset = []
    map1 = texter(address_1, map_type)
    if map_type == 'smb':
        ## create segmets of smb
        for i in range(len(map1[0])-16):
            segment = []
            for row in map1:
...
def points_extractor(mario_addresses, kid_addresses, blending_point):
    ...
    ##create mario dataset
    print(os.listdir(mario_addresses))
    print(os.listdir(kid_addresses))
    for map_name in os.listdir(mario_addresses):
        if map_name[0]=='m':

            dataset_smb = dataset_creator(map_name, 'smb')
    ...
    for key in total_distributions_smb:
            total_distributions_smb[key] /= len(os.listdir(mario_addresses))
            
    ##create kidicarus dataset
    for map_name in os.listdir(kid_addresses):
        if map_name[0]=='k':

            dataset_kid = dataset_creator(map_name, 'kid')
    ...
    for key in total_distributions_kid:
            total_distributions_kid[key] /= len(os.listdir(kid_addresses))
